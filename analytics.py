...
message_regex = re.compile(r'^(\d{2}\/\d{2}\/\d{4}, \d{1,2}:\d{1,2}).(?:pm|am) - ([^:]+): (.+)$')
...
for line in chat_data:
    ...
    if match:
        ...
        member = emoji.demojize(match.group(2), language='en').strip()
        ...
        emojis.append(re.findall(EMOJI_SEQUENCE, match.group(3)))
    else:
...
def split_count(text):
    emoji_list = []
    data = re.findall(EMOJI_SEQUENCE, text)
    for word in data:
        if any(char in text for char in word):
...
emojis = sum([len(em) for em in df['emojis']])
URLPATTERN = r'(https?://\S+|www\.\S+)'
df['urlcount'] = df.message.apply(lambda x: re.findall(URLPATTERN, x))
links = np.sum([1 for url in df['urlcount'] if len(url) > 0])
...
#Clear the HTML file
open('data.html', 'w').close()
with open("data.html", "a") as myfile:
    myfile.write(css)
    myfile.write("<h1>Stats</h1>")
    myfile.write("<p>Messages:" + str(total_messages) + '</p>\n')
    myfile.write('<p>Average message length:'+ str(avg_message_length) + '</p>\n')
    myfile.write("<p>Media messages:"+ str(media_messages) + '</p>\n')
    myfile.write("<p>Emojis:"+ str(emojis) + '</p>\n')
    myfile.write("<p>Links:"+ str(links) + '</p>\n')
...
df['emojis_per_message'] = df['emojis'].apply(lambda x: len(str(x)))
...
with open("data.html", "a", encoding="utf-8") as myfile:
    myfile.write('<div class="container">\n')
    myfile.write(stats_df.to_html())
    myfile.write('\n</div>')
...
word_freq = Counter([b for b in all_words if len(b) > 3])
...
pd.set_option('display.max_rows', emoji_df.shape[0]+1)

with open("data.html", "a", encoding="utf-8") as myfile:
    myfile.write('<div class="container">\n')
    myfile.write(emoji_df.to_html())
    myfile.write('\n</div>')
...
plt.ylabel('Counts')
#plt.show()
plt.savefig('Frequent Words.png')

plt.figure(figsize=[10, 8])
sns.barplot(x='count', y='date', data=df['date'].value_counts().head(20).reset_index(), color='#F0D653')
plt.title('Top 10 days with highest number of Messages')
plt.xlabel('Number of Messages')
plt.ylabel('Date')
#plt.show()
plt.savefig('Top 10 days with highest number of Messages.png')


plt.figure(figsize=[10, 8])
sns.barplot(x='count', y='time', data=df['time'].value_counts().head(20).reset_index(), color='#F0D653')
plt.title('Most Active Times')
plt.xlabel('Number of messages')
plt.ylabel('Time')
#plt.show()
plt.savefig('Most Active times.png')
...
with open("data.html", "a", encoding="utf-8") as myfile:
    myfile.write('<img src="Frequent Words.png" />\n')
    myfile.write('<img src="Most Active times.png" />\n')
    myfile.write('<img src="Top 10 days with highest number of Messages.png" />\n')